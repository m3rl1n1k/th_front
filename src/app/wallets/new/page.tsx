...
import { createWallet, getWalletTypes, getWalletsList } from '@/lib/api';
import { useTranslation } from '@/context/i18n-context';
import { useToast } from '@/hooks/use-toast';
import type { CreateWalletPayload, WalletTypeMap, WalletDetails } from '@/types';
...
const createWalletFormSchema = (t: Function) => z.object({
  ...
  initialBalance: z.coerce.number().optional().nullable(), // User enters normal amount
  currency: z.string().regex(currencyCodeRegex, { message: t('invalidCurrencyCode') }).min(3, { message: t('currencyCodeRequired') }).max(3),
...
export default function NewWalletPage() {
  ...
  const { control, handleSubmit, register, formState: { errors, isSubmitting }, setValue } = useForm<WalletFormData>({
    ...
    defaultValues: {
      name: '',
      initialBalance: null,
      currency: user?.userCurrency?.code || '', // Pre-fill with user's default currency
      type: '',
      accountNumber: null,
    },
  });
  ...
  const anyDataLoading = isLoadingTypes || isLoadingExistingWallets;
          ...
          <CardContent>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="currency">{t('currencyCodeLabel')}</Label>
                  <Input id="currency" {...register('currency')} placeholder={t('currencyCodePlaceholder')} className={errors.currency ? 'border-destructive' : ''} maxLength={3}/>
