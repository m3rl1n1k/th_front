...
import {
  ...
  getDashboardMonthlyExpensesByCategory
} from '@/lib/api';
import { useTranslation } from '@/context/i18n-context';
import { CurrencyDisplay } from '@/components/common/currency-display';
import { DollarSign, TrendingUp, TrendingDown, AlertTriangle, Wallet, PieChart as PieChartIcon, ExternalLink } from 'lucide-react';
import { Skeleton } from '@/components/ui/skeleton';
import { useToast } from '@/hooks/use-toast';
import type { MonthlyExpensesByCategoryResponse, MonthlyExpenseByCategoryItem } from '@/types';
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (isAuthenticated && token) {
      ...
      Promise.all([
        ...
        .catch(error => {
          toast({
            ...
            description: error.message || "Could not load dashboard summary data.",
          });
          setSummaryData(null);
        })
        .finally(() => {
      ...
      getDashboardMonthlyExpensesByCategory(token)
        .then(data => {
        ...
        .catch(error => {
          toast({
            variant: "destructive",
            title: t('errorFetchingData'),
            description: error.message || "Could not load expenses by category data.",
          });
  ...
  const renderSkeletonCard = (title: string, icon?: React.ReactNode, className?: string) => (
  ...
  const renderErrorState = (messageKey: string, spanFull?: boolean) => (
    <Card className={`bg-destructive/10 border-destructive text-destructive-foreground ${spanFull ? 'md:col-span-3' : ''}`}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
